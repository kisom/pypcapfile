...
def _read_a_packet(file_h, hdrp, layers=0):
    ...
    if layers > 0:
        layers -= 1
        ctype_ptr = ctypes.POINTER(linklayer.clookup(hdrp[0].ll_type))
        raw_packet = ctypes.cast(raw_packet_data, ctype_ptr).contents
        raw_packet.__init__(raw_packet_data, layers)
    else:
